...
function LoginPageContent() {
  ...
  const handleSendPhoneOtp = async (e: React.FormEvent) => {
    ...
    const res = await sendFirebasePhoneOtp(phone, "recaptcha-login-page");
    setLoading(false);
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <div id="recaptcha-login-page" />

      <motion.div
        initial={{ opacity: 0, y: 20, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        className="w-full max-w-lg bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-3xl p-8 border border-white/80 dark:border-slate-800 shadow-2xl relative overflow-hidden"
      >
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-brand-500 via-indigo-500 to-purple-500" />

        {/* Brand Header */}
        <div className="text-center space-y-1.5 mb-6">
          <div className="w-12 h-12 mx-auto rounded-2xl bg-gradient-to-tr from-brand-600 to-indigo-600 text-white flex items-center justify-center shadow-lg shadow-brand-500/25 mb-3">
            <Sparkles className="w-6 h-6" />
          </div>
          <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">
            Welcome to TheBook<span className="text-brand-600 dark:text-brand-400">MyVenues</span>
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
            Discover and book premium sports turfs, mini theatres, and banquet halls
          </p>
        </div>
        ...
        {successMsg && (
          <div className="mb-4 p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 text-xs font-semibold flex items-center gap-2">
        ...
        {mode === "SIGNIN" && (
          <form onSubmit={handleSignIn} className="space-y-4">
            <div>
              ...
              <div className="flex items-center px-4 py-3 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus-within:border-brand-500 transition-all">
            ...
            <button
              ...
              className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm rounded-2xl shadow-lg shadow-brand-500/25 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {loading ? (
                <>
        ...
        {/* 2. SIGN UP (EMAIL, PASSWORD, NAME, ROLE) */}
        {mode === "SIGNUP" && (
            ...
            {!isOtpStep ? (
              <form onSubmit={handleSendPhoneOtp} className="space-y-4">
                ...
                <div className="flex items-center rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus-within:border-brand-500 overflow-hidden">
                  <div className="px-4 py-3.5 bg-slate-100 dark:bg-slate-700 text-xs font-bold text-slate-700 select-none">
                    🇮🇳 +91
                  </div>
                  <input
                    type="tel"
                    ...
                    className="w-full px-4 py-3.5 bg-transparent text-slate-900 dark:text-white font-bold text-sm outline-none"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs rounded-2xl shadow-md transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
              ...
              <form onSubmit={handleVerifyPhoneOtp} className="space-y-4">
                ...
                <div className="flex items-center justify-center gap-2">
                  {otpDigits.map((digit, idx) => (
                    <input
                      ...
                      className="w-11 h-14 text-center text-xl font-bold rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:border-brand-500 outline-none"
                    />
                  ))}
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-2xl shadow-md transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
          ...
          <div className="mt-5 space-y-3">
            ...
            <div className="grid gap-2">
              <button
                ...
                disabled={loading}
                className="w-full py-3 px-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 text-slate-700 dark:text-slate-200 font-bold text-xs transition shadow-sm flex items-center justify-center gap-2.5"
              >
